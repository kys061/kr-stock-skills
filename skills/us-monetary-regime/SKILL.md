...
| liquidity_tracker | `track_liquidity()` | 유동성 점수 (0~100, TGA 포함 시 5-컴포넌트) |
...
### TGA (Treasury General Account) 유동성 컴포넌트

TGA 잔고 변화율이 제공되면 유동성 추적이 4→5 컴포넌트로 확장된다.
- TGA 감소 → 재정지출 → 시중 유동성 증가 (점수 상승)
- TGA 증가 → 국채 발행 → 시중 유동성 흡수 (점수 하락)

| 가중치 (5-컴포넌트) | Fed B/S | M2 | DXY | RRP | TGA |
|:--:|:--:|:--:|:--:|:--:|:--:|
| 값 | 0.25 | 0.20 | 0.20 | 0.15 | 0.20 |

- `tga_change_1m_pct=None` (기본): 기존 4-컴포넌트 유지 (하위 호환)
- `tga_change_1m_pct=<float>`: 5-컴포넌트 활성화
