...
한국 주식 시장(KOSPI/KOSDAQ) 분석을 위한 Claude Code 플러그인. 44개 전문 스킬로 시장 분석, 종목 스크리닝, 전략 수립, 포트폴리오 관리까지 워크플로우를 제공합니다.
...
14개 한국 섹터(반도체, 자동차, 조선, 바이오 등)의 성과를 비교하고 경기순환 위치를 판단합니다.
...
펀더멘털(재무/밸류에이션) + 기술적(추세/패턴) + 수급(투자자별) 종합 분석을 한 번에 제공합니다.
...
#### `/kr-strategy-synthesizer` — 7-컴포넌트 확신도 통합

7개 업스트림 스킬 결과(Market Breadth, Uptrend, Market Top, Macro, FTD, VCP, CANSLIM)를 통합하여 0-100 확신도 점수를 산출합니다.
...
4개 시장 상태 + 14 섹터 분석 + 8개 체크리스트로 주간 전략 리포트를 생성합니다.
...
시장/섹터 수급을 7단계 시그널(Strong Buy ~ Strong Sell)로 분류하고, 14 섹터별 HHI(허핀달 지수)로 집중도를 분석합니다.
