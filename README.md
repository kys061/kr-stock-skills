...
한국 주식 시장(KOSPI/KOSDAQ) 분석을 위한 Claude Code 플러그인. 57개 전문 스킬로 시장 분석, 종목 스크리닝, 전략 수립, 포트폴리오 관리까지 워크플로우를 제공합니다.
...
| 기능 | 데이터 소스 | Tier |
|------|------------|------|
| 시세 (OHLCV), 시총, 지수 | KRX Open API | 0 |
| PER/PBR/재무제표/밸류에이션 | yfinance | 1 |
| 투자자별 매매동향 (12분류→3분류) | PyKRX | 2 |
| 공매도 잔고/거래량 | PyKRX | 2 |
| 재무제표 (BS/IS/CF) | DART | 2* |
| 글로벌 지수/환율/원자재 | FDR | 2 |
| FRED 경제지표 | FDR | 2 |
| ETF NAV/괴리율/구성종목 | PyKRX | 2 |
| 국채/회사채 수익률 | PyKRX | 2 |
| 공시/대주주/배당 | DART | 2* |
| 실시간 시세/분봉/호가 | 한투 API | 3 |
...
#### `/kr-stock-selector` — 5조건 종목 선별

Minervini Stage 2 간소화 5조건(200MA 추세, 4중 정배열, 52주 저가 +30%, 52주 고가 -25%, 시총 1,000억)으로 종목을 Pass/Fail 선별합니다.

```
/kr-stock-selector
/kr-stock-selector KOSPI 5조건 통과 종목 스캔
/kr-stock-selector KOSDAQ 시장에서 트렌드 종목 선별
```

**데이터**: KRX Open API → yfinance → pykrx 3단계 폴백
...
#### `/us-indicator-dashboard` — 미국 경제지표 대시보드

미국 경제지표 21개(7개 카테고리)를 자동 수집하고, 5-컴포넌트 레짐 판정 + 한국 시장 영향 분석 + 향후 발표 일정 캘린더를 포함한 4-Section 리포트를 생성합니다.

```
/us-indicator-dashboard
/us-indicator-dashboard 미국 경제지표 종합 분석
```

**21개 지표**: GDP, 기준금리, 국채2Y/10Y, CPI, PCE, PPI, 인플레기대, 실업률, 주당근무시간, 시간당소득, 실질임금, 실업수당, 소매판매, ISM PMI, 소비자심리, 소비자신뢰, 기업재고, 주택착공, 자동차판매, 경상수지

**4-레짐 판정**: Goldilocks / Overheating / Stagflation / Recession

**데이터**: yfinance 2개(국채) + WebSearch 19개 = 21개 지표 | 2분 이내 완료
...
57개 스킬은 **7개 클러스터 + 1개 크로스커팅 모듈**로 구성되며, 데이터 → 분석 → 선별 → 의사결정 파이프라인을 형성합니다.
...
`stock-screener` · `stock-selector` · `pair-trade` · `options-advisor` · `bubble-detector` · `credit-monitor` · `short-sale-tracker` · `institutional-flow` · `technical-analyst` · `market-news-analyst` · `daily-market-check` · `morning-briefing` · `dart-disclosure-monitor` · `skill-reviewer`
...
```
/kr-stock-selector              → 5조건 트렌드 종목 선별
/kr-canslim-screener            → 성장주 CANSLIM 스크리닝
...
```
/kr-supply-demand-analyzer      → 시장/섹터 수급 종합 (15섹터 HHI)
...
/kr-strategy-synthesizer        → 9-컴포넌트 확신도 통합 판단
...
| **담보유지비율** | 140% |
| **15 한국 섹터** | 반도체, 자동차, 조선, 방산, 바이오, 2차전지, 전력기기, 화학, 철강, 건설, 금융, 통신, 유틸리티, 엔터, 원전 |
...
│   ├── kr-market-top-detector/
│   ├── kr-macro-regime/
│   ├── kr-ichimoku-filter/       # Phase 4: 종목 스크리닝 (10개)
...
```
pip install pykrx>=1.0.0 finance-datareader>=0.9.0 opendartreader>=0.2.0 yfinance>=0.2.0 pandas>=2.0.0 numpy>=1.24.0
```
